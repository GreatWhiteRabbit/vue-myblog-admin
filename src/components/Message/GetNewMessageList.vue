<template>
  <div v-loading="loading">
    <el-table v-if="list"
      ...
      >
        <template slot-scope="scope">
          <span style="margin-left: 10px">{{ scope.row.message.blog_id === null ? "无" : scope.row.message.blog_id}}</span>
        </template>
      </el-table-column>
      <el-table-column
          label="用户昵称"
      ...
      >
        <template slot-scope="scope">
          <span style="margin-left: 10px">{{ scope.row.message.second_name }}</span>
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          <span style="margin-left: 10px">{{ scope.row.message.response_user }}</span>
        </template>
      </el-table-column>
      <el-table-column
          label="回复时间"
...
export default {
  ...
  methods: {
    handleEdit(index){
      ...
      if(this.list[index].message.replyed_id === 0) {
        this.form.replyed_id = this.list[index].message.reply_id;
      }
      else {
        this.form.replyed_id = this.list[index].message.replyed_id;
      }
    ...
    getList(){
      ...
      axios({
        method:'get',
        url:'/apis/reply/getNewMessage/' + this.current_page + '/' + 10
      })
          .then(function(res){
            console.log(res)
