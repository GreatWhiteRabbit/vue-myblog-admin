<template>
  <div  class="route" v-show="showThisPage">
...
export default {
  ...
  methods:{
    cancel(){
      this.$router.push("/home/articles/list");
      sessionStorage.setItem("isShowUpdatePage","false");
    },
    onSubmit(formName) {
      // 为表单绑定验证功能
      let that = this;
    ...
    getBlogById() {
      axios({
        url:'/apis/blog/getBlogById/' + sessionStorage.getItem("blog_id"),
        method:'get'
      }).then(res => {
        if(res.data.success === true) {
            this.relativeRUL = res.data.data.blog_cover;
            this.blog_cover = "/apis" + "/blog/static/img/" + this.relativeRUL;
            this.form.title = res.data.data.blog_title;
            this.form.desc = res.data.data.blog_description;
            this.HTMLtext = res.data.data.md_content;
            this.form.label = res.data.data.label_name;
            this.form.classify = res.data.data.category_name;
            this.form.is_show = res.data.data.blog_show;

            sessionStorage.setItem("blog_id",res.data.data.blog_id);
            sessionStorage.setItem("category_id",res.data.data.category_id);
            sessionStorage.setItem("label_id",res.data.data.label_id);
        } else {
  ...
  created(){
    if(sessionStorage.getItem("isShowUpdatePage") === "true") {
